...
// WSFE negocia DHE con una clave Diffie-Hellman de 1024 bits, por debajo del mínimo
// que exige el nivel de seguridad 2 (el default de OpenSSL 3): el handshake muere con
// ERR_SSL_DH_KEY_TOO_SMALL. Bajamos a nivel 1 sólo en este agente, que se usa
// exclusivamente contra los web services de ARCA. WSAA usa ECDHE y no lo necesita,
// pero comparte el agente para que un cambio de config de ARCA no lo rompa.
const agenteArca = new https.Agent({ ciphers: "DEFAULT:@SECLEVEL=1", keepAlive: true });

// fetch() no deja elegir el agente TLS, así que las llamadas SOAP van por node:https.
function postSoap(url, headers, cuerpo) {
  return new Promise((resolve, reject) => {
    const u = new URL(url);
    const req = https.request({
      hostname: u.hostname, port: 443, path: u.pathname + u.search, method: "POST",
      agent: agenteArca,
      headers: { ...headers, "Content-Length": Buffer.byteLength(cuerpo) },
    }, res => {
      let texto = "";
    ...
    req.on("error", reject);
    req.setTimeout(60000, () => req.destroy(new Error("timeout de 60s")));
    req.end(cuerpo);
  });
}
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:wsaa="http://wsaa.view.sua.dvadac.desein.afip.gov">
  ...
  let r;
  try {
    r = await postSoap(WSAA_URL, { "Content-Type": "text/xml; charset=utf-8", SOAPAction: "" }, soap);
  } catch (e) {
    const causa = e.cause?.code || e.code || e.message;
    throw new Error(`WSAA: no se pudo contactar a ${new URL(WSAA_URL).host} (${causa})`);
  }
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ar="http://ar.gov.afip.dif.FEV1/">
  ...
  let r;
  try {
    r = await postSoap(WSFE_URL, {
      "Content-Type": "text/xml; charset=utf-8",
      SOAPAction: `http://ar.gov.afip.dif.FEV1/${metodo}`,
    }, soap);
  } catch (e) {
    // Un error de red a secas no dice nada: la causa (ETIMEDOUT, ENOTFOUND, TLS…)
    // es lo único que distingue un bloqueo de red de una caída del servicio.
    const causa = e.cause?.code || e.code || e.message;
    throw new Error(`WSFE ${metodo}: no se pudo contactar a ${new URL(WSFE_URL).host} (${causa})`);
  }
  const texto = r.texto;
...
// Deja el resultado del barrido en sync_estado (local 'ARCA') para que un fallo del
// cron se vea en el panel de completitud en vez de morir en un console.error.
async function marcarSync(sql, estado, error) {
  const hoy = new Date(Date.now() - 3 * 3600 * 1000).toISOString().slice(0, 10);
  await sql`INSERT INTO sync_estado (fecha, local, estado, intentos, ultimo_error, actualizado_en)
    VALUES (${hoy}, 'ARCA', ${estado}, 1, ${error}, now())
    ON CONFLICT (fecha, local) DO UPDATE
      SET estado = ${estado}, intentos = sync_estado.intentos + 1,
          ultimo_error = ${error}, actualizado_en = now()`;
}

async function sincronizarEmitidos(sql, opciones = {}) {
  try {
    const r = await barrerEmitidos(sql, opciones);
    if (r.ok) await marcarSync(sql, "ok", null);
    return r;
  } catch (e) {
    await marcarSync(sql, "error", String(e.message).slice(0, 500)).catch(() => {});
    throw e;
  }
}

async function barrerEmitidos(sql, { maxComprobantes = 400 } = {}) {
