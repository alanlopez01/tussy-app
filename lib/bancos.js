...
function parsearGalicia(texto) {
  ...
  // Último saldo del extracto (la columna Saldo es acumulada)
  let saldoFinal = null;
  if (idx.saldo >= 0) {
    for (let i = lineas.length - 1; i > 0 && !saldoFinal; i--) {
      const c = partirCSV(lineas[i]);
      const v = aNumeroAR(c[idx.saldo]);
      const f = (c[idx.fecha] || "").match(/^(\d{2})\/(\d{2})\/(\d{4})/);
      if (v && f) saldoFinal = { cuenta: "galicia", fecha: `${f[3]}-${f[2]}-${f[1]}`, saldo: v };
    }
  }
  return { filas, saldo: saldoFinal };
}
