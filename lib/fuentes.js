...
// Cómo se cobró una venta. Los locales lo etiquetan distinto según el sistema:
// Woo usa "Chip and Pin" / "Dinero en efectivo" / "Cash"; Dragonfish, "ELECTRON"
// (tarjeta o QR) / "PESOS" (efectivo). Se normaliza a electronico | efectivo | otro.
function normalizarMedio(etiqueta) {
  const s = String(etiqueta || "").toUpperCase();
  if (/ELECTRON|CHIP|TARJET|DEBIT|CREDIT|QR|MERCADO ?PAGO|POINT|VISA|MASTER|POSNET|PAYWAY|TRANSFEREN/.test(s)) return "electronico";
  if (/EFECTIVO|CASH|PESOS|CONTADO/.test(s)) return "efectivo";
  return "otro";
}
  ...
  try {
    while (true) {
      ...
      for (const order of orders) {
        // Woo guarda un solo medio por orden (payment_method_title)
        const etiqueta = order.payment_method_title || order.payment_method || "";
        const totalOrden = parseFloat(order.total || 0);
        if (totalOrden) {
          cobros.push({
            fecha, local: local.nombre, orden_id: String(order.id), item: 1,
            medio: normalizarMedio(etiqueta), detalle: etiqueta || null, monto: totalOrden,
          });
        }
    ...
    return { ok: true, filas, cobros, error: null };
  } catch (e) {
    return { ok: false, filas: [], cobros: [], error: e.message };
  }
}
  ...
  try {
    while (true) {
      ...
      for (const order of orders) {
        ...
        // El canal online cobra siempre por medio electrónico (PagoNube, MercadoPago
        // o transferencia). El gateway y las cuotas son lo que después permite
        // calcular el costo real de cada venta sin depender del reporte mensual.
        cobrosTN.push({
          fecha, local: "Tiendanube", orden_id: ordenId, item: 1, medio: "electronico",
          detalle: order.gateway_name || order.gateway || "Online", monto: parseFloat(order.total || 0),
          gateway: order.gateway || null,
          cuotas: parseInt(order.payment_details?.installments) || 1,
        });
        // Cliente de la orden, para recompra/LTV. El token no expone customer.id,
        // pero el email de contacto identifica igual: se guarda solo un hash
        // (63 bits del SHA-256), nunca el email.
        let clienteId = order.customer?.id || null;
        if (!clienteId && order.contact_email) {
          const h = crypto.createHash("sha256").update(String(order.contact_email).trim().toLowerCase()).digest("hex");
          clienteId = BigInt("0x" + h.slice(0, 15)).toString();
        }
        clientesTN.push({
          orden_id: ordenId, cliente_id: clienteId,
          fecha, total: parseFloat(order.total || 0),
        });
    ...
    return { ok: true, filas, cobros: cobrosTN, clientes: clientesTN, error: null };
  } catch (e) {
...
// Cobros de una factura Dragonfish: ValoresDetalle trae una línea por medio
// (una venta puede pagarse parte con ELECTRON y parte en PESOS).
function cobrosDeFactura(fac, fecha, localNombre) {
  const ordenId = `${fac.Letra || ""}${fac.PuntoDeVenta || ""}-${fac.Numero || ""}`;
  const valores = (fac.ValoresDetalle || [])
    .map((v, i) => ({ i, monto: parseFloat(v.Monto || 0), etiqueta: v.ValorDetalle || v.Valor || "" }))
    .filter(v => v.monto);
  if (!valores.length) return [];
  // El Monto del valor es lo que se le cobró a la tarjeta e incluye el recargo por
  // cuotas, así que suele superar el Total de la factura. Se prorratea para que la
  // suma de cobros sea igual a la venta y ambos lados comparen contra lo mismo.
  const totalFactura = parseFloat(fac.Total || 0);
  const sumaValores = valores.reduce((a, v) => a + v.monto, 0);
  const factor = totalFactura > 0 && sumaValores > 0 ? totalFactura / sumaValores : 1;
  return valores.map(v => ({
    fecha, local: localNombre, orden_id: ordenId, item: v.i + 1,
    medio: normalizarMedio(v.etiqueta), detalle: v.etiqueta || null,
    monto: Math.round(v.monto * factor * 100) / 100,
  }));
}
  ...
  try {
    while (true) {
      ...
      for (const fac of resultados) {
        ...
        if (!cobrosPorDia[dia]) cobrosPorDia[dia] = [];
        cobrosPorDia[dia].push(...cobrosDeFactura(fac, dia, local.nombre));
      }
      if (resultados.length < 50 || hayMasViejas) break;
      page++;
    }
    return { ok: true, porDia, cobrosPorDia, error: null };
  } catch (e) {
    return { ok: false, porDia, cobrosPorDia, error: e.message };
  }
}
...
async function fetchDFDia(local, fecha) {
  ...
  return { ok: r.ok, filas: r.porDia[fecha] || [], cobros: (r.cobrosPorDia || {})[fecha] || [], error: r.error };
}

module.exports = { wooLocales, dfLocales, fetchWooDia, fetchTNDia, fetchDFDia, fetchDFRango, diaSiguiente, normalizarMedio };
