    ...
    while (true) {
      ...
      for (const order of orders) {
        ...
        for (const item of order.products || []) {
          ...
          filas.push({
            fecha, local: "Tiendanube", sistema: "Tiendanube", orden_id: ordenId,
        ...
        if (shipping > 0) filas.push({ fecha, local: "Tiendanube", sistema: "Tiendanube", orden_id: ordenId, producto: "ENVIO", sku: "", color: "", talle: "", cantidad: 1, precio_unit: Math.round(shipping), total: Math.round(shipping) });
        if (descuento > 0) filas.push({ fecha, local: "Tiendanube", sistema: "Tiendanube", orden_id: ordenId, producto: "DESCUENTO", sku: "", color: "", talle: "", cantidad: 1, precio_unit: -Math.round(descuento), total: -Math.round(descuento) });
        const diferencia = totalOrden - (subtotalProductos + shipping - descuento);
        if (Math.abs(diferencia) > 1) filas.push({ fecha, local: "Tiendanube", sistema: "Tiendanube", orden_id: ordenId, producto: "AJUSTE", sku: "", color: "", talle: "", cantidad: 1, precio_unit: Math.round(diferencia), total: Math.round(diferencia) });
      }
...
function filasDeFactura(fac, fecha, localNombre) {
  const ordenId = `${fac.Letra || ""}${fac.PuntoDeVenta || ""}-${fac.Numero || ""}`;
  const filas = (fac.FacturaDetalle || []).map(item => ({
    fecha, local: localNombre, sistema: "Dragonfish", orden_id: ordenId,
  ...
  try {
    while (true) {
      ...
      for (const fac of resultados) {
        ...
        porDia[dia].push(...filasDeFactura(fac, dia, local.nombre));
      }
