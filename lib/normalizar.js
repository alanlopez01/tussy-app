...
const CATEGORIAS = ["REMERA","REMERON","POLO","POLERA","PANTALON","PANTALONES","BUZO","CAMPERA",
  ...
  "BOLSO","BOLSOS","MOCHILA","CARTERA","CARTERAS","PILUSO","PANT","PANTS"];
...
function normalizarProducto(nombre) {
  ...
  // Sinónimos: "PANT DOPPLER" (Dragonfish) y "PANTALON DOPPLER" son el mismo modelo
  const categoriaSing = categoria.replace(/ES$/, "").replace(/^REMERON$/, "REMERA").replace(/^PANTS?$/, "PANTALON");
  ...
  let modeloStr = modelo.join(" ");
  // Si al ignorar la marca no queda modelo ("BOLSO TSSY" → "BOLSO"), la marca ES
  // el modelo y se conserva: así el listado no muestra un genérico irreconocible.
  if (!modeloStr && categoriaSing && palabras.some(p => p === "TSSY" || p === "TSSYA")) {
    modeloStr = "TSSY";
  }
