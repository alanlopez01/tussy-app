...
// El conjunto no se limita a la categoría NEW IN: con 49 productos el mismo
// público ve lo mismo una y otra vez y la frecuencia se dispara. Entra todo lo
// publicado con stock salvo el merchandising barato — el filtro de
// disponibilidad de Meta saca solo lo que se agota.
...
async function actualizarConjuntoNewIn() {
  ...
  for (let page = 1; page <= 6; page++) {
    ...
    for (const p of lote) {
      ...
      if (cats.some(c => NEWIN_CATEGORIAS_EXCLUIDAS.has(c))) continue;
      // stock null = producción a demanda: siempre comprable
      const stock = (p.variants || []).reduce(
        (a, v) => a + (v.stock === null ? 1 : Number(v.stock) || 0), 0);
      if (stock > 0) ids.push(String(p.id));
    }
    if (lote.length < 100) break;
  }
  if (!ids.length) return { ok: false, motivo: "TN devolvió 0 productos NEW IN, no piso el filtro" };
  // AND explícito: con varias claves sueltas Meta interpreta OR y el conjunto
  // termina abarcando todo el catálogo con stock (pasó el 18/8).
  const body = new URLSearchParams({
    filter: JSON.stringify({ and: [
      { retailer_product_group_id: { is_any: ids } },
