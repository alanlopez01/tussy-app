...
// Solo los locales con inventario real (ver nota arriba)
function fuentesStock(env = process.env) {
  return dfLocales(env).map(l => ({ local: l.nombre, fn: () => fetchStockDF(l) }));
}

// Locales cuyo stock no es confiable, para poder avisarlo en la app
const LOCALES_SIN_STOCK = ["Palermo", "La Plata"];
const MOTIVO_SIN_STOCK = {
  "Palermo": "WooCommerce sin gestión de inventario",
  "La Plata": "WooCommerce sin gestión de inventario",
