...
// Tiendanube cobra 0,5% + IVA sobre toda transacción que NO pase por PagoNube:
// da igual si es transferencia o MercadoPago. Con PagoNube no se cobra aparte,
// ya está dentro de su tasa.
const COMISION_PLATAFORMA_TN = 0.005 * 1.21;
...
function margenDeOperacion(fila, ctx) {
  ...
  if (real) {
    // Pago de MercadoPago dentro de Tiendanube: su comisión más el 0,5% de la
    // plataforma por usar un gateway externo.
    const plataforma = fila.local === "Tiendanube" ? porMp * COMISION_PLATAFORMA_TN : 0;
    comisiones = Math.round(Number(fila.financiero_real) + plataforma);
    ...
    // La transferencia no paga gateway pero sí el 0,5% de la plataforma,
    // sobre el monto transferido (que no entra en porMp).
    comisiones = Math.round(transferencia * COMISION_PLATAFORMA_TN);
    exacto = true;
  } else if (gateway === "free") {
    comisiones = 0;
    exacto = true;
  } else if (gateway === "mercado-pago") {
    // Sin el pago cruzado todavía: tasa medida de MercadoPago + el 0,5% de la plataforma.
    comisiones = Math.round(porMp * ((ctx.tasas.Tiendanube?.pct || 0) + COMISION_PLATAFORMA_TN));
  } else {
    ...
    // Si no entró un peso por MercadoPago no hay nada que estimar: la comisión
    // es cero y es exacta. Marcarla como estimada ensuciaba la pantalla con
    // tickets de efectivo que no tienen ninguna incertidumbre.
    if (porMp === 0) exacto = true;
  }
  ...
  return {
    ...
    origen_comision: real ? "mercadopago"
                     : gateway === "pago-nube" ? "pagonube"
                     : gateway === "offline" ? "transferencia"
                     : gateway === "free" ? "free"
                     : porMp === 0 ? "sin_cobro_electronico"
                     : "estimada",
