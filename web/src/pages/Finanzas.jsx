...
export default function Finanzas() {
  ...
  const gastos = Object.entries(data?.porCatGasto || {}).sort((a, b) => b[1] - a[1]).slice(0, 10);
      ...
      {error ? (
        ...
        <>
          <div className="bg-negro text-white rounded-lg p-6">
            ...
            <div className="text-[32px] leading-tight font-bold mt-1.5 tabular-nums">{fmtPesos(data.saldoActual || 0)}</div>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
            <Card title={`Gastos por categoría · ${MESES[mes - 1]}`}>
            ...
            <Card title="Movimientos de hoy">
              {!data.txHoy?.length ? <p className="text-[13px] text-ink-3 py-4 text-center">Sin movimientos hoy</p> : (
